<?php
	...
	class Db {
		...
		function connect() {
			$this->linkid = @mysqli_connect($this->hostname, $this->username, $this->password);
			if(!$this->linkid){
				$this->error("Could not connect with server!");
				return false;
			} else {
				mysqli_set_charset($this->linkid,"utf8");
				$check = @mysqli_select_db($this->linkid,$this->databasename );
				if ($check) {
					return $this->linkid;
				} else {
		...
		function lastInsert($result){
			$insertid = NULL;
			$insertid = @mysqli_insert_id($this->linkid);
			return $insertid;
		} 
		...
		*/
		function error($message) {
			$this->error = $message.' '.mysqli_error($this->linkid).'.';
			echo $this->error;
		} // End function error
		...
		function   update($info)
		 ...
		 function q($str = null)
	     {
		   return "'" . mysqli_escape_string($this->linkid,$str) . "'";
	     }
